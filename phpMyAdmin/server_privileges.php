...
$strPrivDescFile = __('Allows importing wei_xin from and exporting wei_xin into files.');
...
function PMA_extractPrivInfo($row = '', $enableHTML = false)
{
    $grants = array(
        ...
        array(
            'File_priv',
            'FILE',
            __('Allows importing wei_xin from and exporting wei_xin into files.')),
        array(
            'References_priv',
            'REFERENCES',
...
function PMA_displayPrivTable($db = '*', $table = '*', $submit = true)
{
    ...
    if (! empty($columns)) {
        ...
        //
        $privTable_names = array(0 => __('wei_xin'), 1 => __('Structure'), 2 => __('Administration'));
        ...
        if ($db == '*') {
            $privTable[0][] = array('File', 'FILE', __('Allows importing wei_xin from and exporting wei_xin into files.'));
        }
